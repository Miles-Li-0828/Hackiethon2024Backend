...
class Script:
    ...
    def calculate_best_move(player, enemy, player_projectiles, enemy_projectiles):
        ...
        proj_type = get_projectile_type(enemy_projectiles)
        proj_distance = abs(get_pos(player)[0] - get_proj_pos(enemy_projectiles)[0])
        ...
        my_cooldown = [primary_on_cooldown(player), secondary_on_cooldown(player), heavy_on_cooldown(player)]

        enemy_cooldowns = [primary_on_cooldown(enemy), secondary_on_cooldown(enemy), heavy_on_cooldown(enemy)]
        ...
        if (distance < 2 and (is_enemy_blocked != 0)):
            if (not (my_cooldown[0])):
                return random.choice([JUMP_FORWARD, JUMP_BACKWARD, PRIMARY, PRIMARY, PRIMARY, PRIMARY, LIGHT])
            elif (not (my_cooldown[2])):
                return random.choice([JUMP_FORWARD, JUMP_BACKWARD, HEAVY, HEAVY, HEAVY, HEAVY, LIGHT])
            else:
                return random.choice([JUMP_FORWARD, JUMP_BACKWARD, LIGHT])
        elif (distance < 2 and (is_enemy_blocked == 0)):
            if (not (my_cooldown[0])):
                return random.choice([BLOCK, BLOCK, JUMP_FORWARD, JUMP_FORWARD, JUMP_BACKWARD, JUMP_BACKWARD, PRIMARY, PRIMARY, PRIMARY, PRIMARY, LIGHT])
            elif (not (my_cooldown[2])):
                return random.choice([BLOCK, BLOCK, JUMP_FORWARD, JUMP_FORWARD, JUMP_BACKWARD, JUMP_BACKWARD, HEAVY, HEAVY, HEAVY, HEAVY, LIGHT])
            else:
                return random.choice([BLOCK, BLOCK, JUMP_FORWARD, JUMP_FORWARD, JUMP_BACKWARD, JUMP_BACKWARD, LIGHT])
        elif (distance >= 2):
            if (not (my_cooldown[1])):
                return random.choice([JUMP_FORWARD, JUMP_BACKWARD, SECONDARY, FORWARD, BACK])
            else:
                return random.choice([JUMP_FORWARD, JUMP_BACKWARD, FORWARD, BACK])
